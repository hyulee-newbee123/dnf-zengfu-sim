...
function pushCommit(sha, parent) {
  ...
  ghApi("PATCH", `${REPO}/git/refs/heads/master`, { sha: commit.sha, force: true });
  return commit.sha;
}

let remote;
try {
  remote = ghApi("GET", `${REPO}/git/ref/heads/master`).object.sha;
} catch {
  remote = null;
}
...
if (!remote || !hasCommit(remote)) {
  const next = pushCommit(head, remote);
  console.log("已用当前提交覆盖远端 master", head.slice(0, 7), "→", next.slice(0, 7));
  console.log("完成", next);
  process.exit(0);
}
